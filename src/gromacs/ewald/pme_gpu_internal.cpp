...
static void pme_gpu_init(gmx_pme_t* pme, const gmx_device_info_t* gpuInfo, PmeGpuProgramHandle pmeGpuProgram)
{
    ...
    //pme_gpu_alloc_energy_virial(pmeGpu); //we move it to the pme_gpu_reinit function because we need the 
                                           //grid information to alloc extra gpu memory for eche pme_solve
                                           //kernel block in the hip version.
...
void pme_gpu_reinit(gmx_pme_t* pme, const gmx_device_info_t* gpuInfo, PmeGpuProgramHandle pmeGpuProgram)
{
    ...
    pme_gpu_alloc_energy_virial(pme->gpu); //we put it here because we need the infomation of grids
